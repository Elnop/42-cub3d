...
/*   Updated: 2023/12/17 05:19:44 by lperroti         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_wall(t_app *papp, t_ray *rays, size_t *i, t_image img)
{
	...
	end_wall = *i;
	while (end_wall < array_size(rays) && rays[*i].wx == rays[end_wall].wx
		&& rays[*i].wy == rays[end_wall].wy
	...
	while (pos.x < end_wall)
	{
		if (!rays[(int)pos.x].ray_mod)
			...
			wall_h = WIN_H / (rays[(int)pos.x].dist + (!rays[(int)pos.x].dist));
		s = (t_coor){(double)t.w / (end_wall - *i), (double)t.h / wall_h};
		pos.y = -1;
		while (++pos.y < wall_h)
