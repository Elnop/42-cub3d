...
void	draw_rect(t_image win_image, t_coo s, t_coo lengths, int color)
{
	size_t	y;
	size_t	x;
...
void	draw_rays(t_app *papp, t_image img)
{
	...
	while (i < array_size(rays))
	{
		draw_line(img,
			...
			(t_coo){
			floor(rays[i].wx * papp->tile_w) + 10,
			floor(rays[i].wy * papp->tile_h) + 10},
			0xFF);
		i++;
	}
}
...
void	draw_minimap(t_app *papp, t_image img)
{
	...
	while (i.y < array_size(papp->map))
	{
		...
		while (i.x < lp_strlen(papp->map[(int)i.y]))
		{
			if (papp->map[(int)i.y][(int)i.x] == '1')
				draw_rect(img, (t_coo){i.x * papp->tile_w + 10,
					i.y * papp->tile_h + 10},
					(t_coo){papp->tile_w, papp->tile_h}, 0xFFFFFF);
			if (papp->map[(int)i.y][(int)i.x] == '0')
				draw_rect(img, (t_coo){i.x * papp->tile_w + 10,
					i.y * papp->tile_h + 10},
					(t_coo){papp->tile_w, papp->tile_h}, 0x666666);
			i.x++;
		}
		i.y++;
	}
	draw_rays(papp, img);
	draw_rect(img,
		(t_coo){papp->p.x * papp->tile_w - papp->tile_w * MOVE_STEP / 2 + 10,
		papp->p.y * papp->tile_h - papp->tile_h * MOVE_STEP / 2 + 10},
		(t_coo){papp->tile_w * MOVE_STEP, papp->tile_h * MOVE_STEP}, 0xFFFF);
}
